...
for sheet_name in sheets:
	...
	ws1.append(["Tecnologia","Equipo","Ip","Vlan","Clientes"])
...
for row in range(1,nrows+1):
  ...
  tecnologia=worksheet.cell(row=row, column=4).value
  nodo=worksheet.cell(row=row, column=5).value
  ip=worksheet.cell(row=row, column=6).value
  vlan=worksheet.cell(row=row, column=8).value
  clientes=worksheet.cell(row=row, column=9).value
  if ciudad == "TIJUANA" or ciudad == "TECATE" or ciudad == "ROSARITO" :
    if "GPON HUAWEI" in tecnologia :
      ...
      for j in find_column.iter_rows():
        ...
        if j[1].value == worksheet.cell(row=row, column=22).value :
          ...
          break
      if found == 0 :    
        wb["GPON HUAWEI"].append([tecnologia,worksheet.cell(row=row, column=22).value,ip])
    if "GPON ISAM" in tecnologia :
      ...
      for j in find_column.iter_rows():
        ...
        if j[1].value == worksheet.cell(row=row, column=22).value :
          ...
          break
      if found == 0 :    
        wb["GPON ISAM"].append([tecnologia,worksheet.cell(row=row, column=22).value,ip])
    if "IP HUAWEI" in tecnologia :
      wb["HUAWEI"].append([tecnologia,nodo,ip,vlan,clientes])
    if "7330" in tecnologia :
      ...
      wb[ciudad+"-7330"].append([tecnologia,nodo,ip,vlan,clientes])
    if text in tecnologia or "7330" in tecnologia : 
      i = i +1
      ...
      ws.cell(row=i, column=1).value=nodo #asignar valor del nombre del nodo
      ws.cell(row=i, column=2).value=ip  #asignar valor de ip
      ws.cell(row=i, column=3).value=tecnologia #asignar valor de tecnologia
      ...
      wb[ciudad].append([tecnologia,nodo,ip,vlan,clientes]) #agregar a hoja con nombre ciudad valor de tecnologia,nodo e ip
